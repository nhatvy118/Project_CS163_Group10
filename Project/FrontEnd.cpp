	...
	while (!WindowShouldClose()) {
		...
		if (isDisplayingResult) {
			if (!SearchDefMode) {
				...
				if (confirmDelete)
				{
					...
					if (CheckCollisionPointRec(mousePoint, { 542,554,161,49 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
						...
						if (DictType == "ENG-ENG")
						{
							...
							for (auto x : tmp)
							{
								trieNode* def = find(EngEngDef, x);
								if (def->mean.size() == 1) DeleteAWord(EngEngDef, x, 0,tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
											break;
										}
									}
								}
						...
						else if (DictType == "ENG-VIE")
						{
							...
							for (auto x : tmp)
							{
								trieNode* def = find(EngVieDef, x);
								if (def->mean.size() == 1) DeleteAWord(EngVieDef, x, 0, tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
						...
						else if (DictType == "VIE-ENG")
						{
							...
							for (auto x : tmp)
							{
								trieNode* def = find(VieEngDef, x);
								if (def->mean.size() == 1) DeleteAWord(VieEngDef, x, 0, tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
						...
						else if (DictType == "SLANG")
						{
							...
							for (auto x : tmp)
							{
								trieNode* def = find(SlangDef, x);
								if (def->mean.size() == 1) DeleteAWord(SlangDef, x, 0, tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
						...
						else if (DictType == "EMOJI")
						{
							...
							for (auto x : tmp)
							{
								trieNode* def = find(EmojiDef, x);
								if (def->mean.size() == 1) DeleteAWord(EmojiDef, x, 0, tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
				...
				DrawLine(Vocab.x + 10, Vocab.y + Vocab.height - 2, Vocab.x + 1400, Vocab.y + Vocab.height - 2, yellow);
				DrawLine(Vocab.x + 10, Vocab.y + Vocab.height - 1, Vocab.x + 1400, Vocab.y + Vocab.height - 1, yellow);
				DrawRectangleRounded({ Vocab.x + 1360,Vocab.y,49,49 }, 0.3, 0, yellow);
				DrawTexture(trashcan, Vocab.x + 1370, Vocab.y + 10, WHITE);

				if (CheckCollisionPointRec(mousePoint, { Vocab.x + 1360,Vocab.y,49,49 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
				{
					confirmDelete = true;
				}
				if (confirmDelete)
				{
					...
					if (CheckCollisionPointRec(mousePoint, { 542,554,161,49 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)){
						if (DictType == "ENG-ENG")
						{
							DeleteAWord(EngEngDef, str, 0, tmp);
							vector <string> tmp2;
							for (auto x : tmp)
							{
								trieNode* def = find(EngEng, x);
								if (def->mean.size() == 1) DeleteAWord(EngEng, x, 0, tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
						...
						{
							DeleteAWord(EngVieDef, str, 0, tmp);
							vector <string> tmp2;
							for (auto x : tmp)
							{
								trieNode* def = find(EngVie, x);
								if (def->mean.size() == 1) DeleteAWord(EngVie, x, 0, tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
						...
						{
							DeleteAWord(VieEngDef, str, 0, tmp);
							vector <string> tmp2;
							for (auto x : tmp)
							{
								trieNode* def = find(VieEng, x);
								if (def->mean.size() == 1) DeleteAWord(VieEng, x, 0, tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
						...
						{
							DeleteAWord(SlangDef, str, 0, tmp);
							vector <string> tmp2;
							for (auto x : tmp)
							{
								trieNode* def = find(Slang, x);
								if (def->mean.size() == 1) DeleteAWord(Slang, x, 0, tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
						...
						{
							DeleteAWord(EmojiDef, str, 0, tmp);
							vector <string> tmp2;
							for (auto x : tmp)
							{
								trieNode* def = find(Emoji, x);
								if (def->mean.size() == 1) DeleteAWord(Emoji, x, 0, tmp2);
								else {
									for (int i = 0; i < def->mean.size(); ++i) {
										if (def->mean[i] == str) {
											def->mean.erase(def->mean.begin() + i);
			...
			if (chooseEE) {
				if (!SearchDefMode) {
					...
					for (int i = 0; i < recommendResult.size(); ++i) {
						...
						if (CheckCollisionPointRec(mousePoint, { 754, recommendBoxPosY + 54 * (float)i,708,54 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && !CheckCollisionPointRec(mousePoint, ActualSearchBar.textbox)) {
							ans.resize(0);
		...
		if (isUpdatingDef) {
			...
			if (chooseEE) {
				if (CheckCollisionPointRec(mousePoint, { 675,877,162,60 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && changeDef.lettercount <100)
				{
				...
				else if (CheckCollisionPointRec(mousePoint, { 675,877,162,60 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && changeDef.lettercount >= 100) {
					tooMuchChar = true;
				}
			}
			if (chooseEV) {
		...
		if (resetData) {
			...
			if (CheckCollisionPointRec(mousePoint, { 542,534,161,49 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
			{
				...
				DeleteAllTree(SlangDef);
				DeleteAllTree(EmojiDef);
				readFileToTree(VieEng, VieEngDef, "VieEngOriginal.txt");
				readFileToTree(EngEng, EngEngDef, "EngEngOriginal.txt");
				readFileToTree(EngVie, EngVieDef, "EngVieOriginal.txt");
				readFileToTree(Emoji, EmojiDef, "EmojiOriginal.txt");
				readFileToTree(Slang, SlangDef, "SlangOriginal.txt");
		...
		if (deleteSuccesful) {
			DrawRectangleRounded({ 501,375,511,230 }, 0.08, 0, blue);
			DrawTextEx(bold, "Delete Data Succesfully", { 565,450 }, 48, 0, GREEN);
			DrawTexture(closeIcon, 966, 393, WHITE);
			if (CheckCollisionPointRec(mousePoint, { 966,393,24,24 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
		...
		if (tooMuchChar) {
			DrawRectangleRounded({ 501,375,511,230 }, 0.08, 0, blue);
			DrawRectangleRoundedLines({ 501,375,511,230 }, 0.08, 0, 2, yellow);
			DrawTextEx(bold, "New Def must < 100 chars", { 550,450 }, 48, 0, RED);
			DrawTexture(closeIcon, 966, 393, WHITE);
			if (CheckCollisionPointRec(mousePoint, { 966,393,24,24 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
				tooMuchChar = false;
			}
...
void AddWordPage(const int screenWidth, const int screenHeight, trieNode*& VieEng, trieNode*& EngVie, trieNode*& EngEng, trieNode*& VieEngDef, trieNode*& EngVieDef, trieNode*& EngEngDef, trieNode*& Emoji, trieNode*& EmojiDef, trieNode*& Slang, trieNode*& SlangDef, Node* favor[], Node* history[]) {
	...
	while (!WindowShouldClose()) {
		...
		if (CheckCollisionPointRec(mousePoint, { 667, 340, 751, 60 }) || CheckCollisionPointRec(mousePoint, { 667, 534, 751, 60 })) addSuccessfully = 1;
		if (CheckCollisionPointRec(mousePoint, chooseDictBox) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
			choose = true;
			chooseEE = false;
			chooseEV = false;
		...
		if (CheckCollisionPointRec(mousePoint, { 969,643,162,60 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
		{
			...
			else if (DefBar.text[0] == '\0') {
				addSuccessfully = 4;
			}
			else if (!chooseEE && !chooseEV && !chooseVE && !chooseSlang && !chooseEmo ) {
				addSuccessfully = 5;
			}
			else
			{
		...
		if (addSuccessfully == 3) DrawTextEx(bold, "Successful !", { 1000 - 65, 800 - 30 }, 60, 0, GREEN);
		else if (addSuccessfully == 2) DrawTextEx(bold, "Please input the word", { 840, 800 - 30 }, 60, 0, RED);
		else if (addSuccessfully == 4) DrawTextEx(bold, "Please input the definition", { 800, 800 - 30 }, 60, 0, RED);
		else if (addSuccessfully == 5) DrawTextEx(bold, "Please choose the dataset to add", { 730, 800 - 30 }, 60, 0, RED);
		EndDrawing();
	}
	ofstream fout;
	fout.open("../Dataset/EngEng.txt");
